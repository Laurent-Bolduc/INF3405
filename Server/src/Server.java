		...
		private void downloadCommand(DataOutputStream out, String[] inputs) throws Exception {
			...
			if (inputs.length == 1) {
				...
				if (!(file.isFile())) {
					...
					// create the output stream of the data file from the socket 
					DataOutputStream dos = new DataOutputStream(socket.getOutputStream());
					// get the input stream 
					FileInputStream fis = new FileInputStream(file.toString());

					int fileDataSize = fis.read();
					int read = 0;

					// while there are bytes in the fis, we read them and write them in the dos
					while (fileDataSize > 0 && (read = fis.read(buff)) > 0) {
						dos.write(buff, 0, read);
						fileDataSize -= read;
					}
					// close the the fis and the dos
					fis.close();
					dos.close();
